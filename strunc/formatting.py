...
from strunc.modes import FormatMode, PrecMode, SignMode
...
def get_mantissa_exp(num: float,
    ...
    if num == 0:
        ...
        if exp is not None:
            if exp != 0:
                warnings.warn('Attempt to set exponent explicity in fixed point exponent mode. '
                              'coercing exponent to 0.')
        exp = 0
        ...
        if exp is not None:
            if exp % 3 != 0:
                warnings.warn(f'Attempt to set exponent explicity to a non-integer multiple of 3 '
                              f'in engineering mode. Coercing to the next lower multiple of 3.')
                exp = (exp // 3) * 3
            if alternate_mode:
                logger.debug(f'Alternate mode ignored when setting exponent explicitly.')
        else:
            exp = floor(log10(abs(num)))
...
def get_exp_str(exp: int, format_mode: FormatMode, capital_exp_char: bool) -> str:
...
def get_round_digit(top_digit: int, bottom_digit: int,
    ...
    # TODO: Decide on default precision/sig figs.  Minimum round-trippable or hard-coded to 6?
