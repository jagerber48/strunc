...
from strunc.modes import FormatMode, PrecMode, SignMode, GroupingMode
from strunc.formatting import (get_mantissa_exp, get_exp_str, get_top_and_bottom_digit,
                               get_round_digit, format_float_by_top_bottom_dig)
from strunc.grouping import add_group_chars_float
...
def parse_format_spec(fmt: str) -> FormatSpec:
    ...
    if prec:
        ...
        if prec < 0 and prec_mode is PrecMode.SIG_FIG:
            raise ValueError(f'Invalid format specifier: \'{fmt}\'. Cannot specify negative '
                             f'number of significant figures.')
...
def pformat_float(num: float, format_spec: FormatSpec) -> str:
    ...
    mantissa_str = format_float_by_top_bottom_dig(mantissa, top_padded_digit,
                                                  round_digit, sign_mode, fill_char)
    ...
    mantissa_str = add_group_chars_float(mantissa_str, grouping_char_1, grouping_char_2,
                                         group_size=3)
